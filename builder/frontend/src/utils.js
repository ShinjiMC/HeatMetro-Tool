...
export function assignCoverageColorsRecursive(nodes, parentTarget = null) {
  ...
  if (parentTarget === null) {
    nodes.forEach((node) => {
      ...
      if (node.children) {
        assignCoverageColorsRecursive(node.children, node.coverage);
      }
    });
    return;
  }
  ...
  if (parentTarget === 100) {
    nodes.forEach((node) => {
      node.coverage = 100;
      if (node.children) {
        assignCoverageColorsRecursive(node.children, 100);
      }
    });
    return;
  }
